...
class QuotationCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['customer', 'quotation_date', 'valid_until', 'tax_rate', 'notes']
...
class QuotationUpdateView(LoginRequiredMixin, UpdateView):
    model = Quotation
    template_name = 'sales/quotation/form.html'
    fields = ['customer', 'status', 'quotation_date', 'valid_until', 'tax_rate', 'notes']
...
class InvoiceCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['customer', 'quotation', 'invoice_date', 'due_date', 'tax_rate', 'notes']
...
class InvoiceUpdateView(LoginRequiredMixin, UpdateView):
    model = Invoice
    template_name = 'sales/invoice/form.html'
    fields = ['customer', 'status', 'invoice_date', 'due_date', 'tax_rate', 'notes']
...
try:
    # Try to register DejaVu Sans font which has better Unicode support
    pdfmetrics.registerFont(TTFont('DejaVuSans', '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'))
    ARABIC_FONT = 'DejaVuSans'
except:
    try:
        # Try Noto Sans Arabic font
    ...
    if request.method == 'POST':
        try:
            ...
            for item_data in items_data:
                if item_data.get('description') and item_data.get('quantity'):
            ...
            invoice.discount_percentage = discount_value if discount_type == 'percentage' else Decimal('0')
...
def search_customers(request):
    ...
    for customer in customers:
        results.append({
            ...
            'company': customer.company or '',
            'email': customer.email,
            'display_name': f"{customer.name} - {customer.company}" if customer.company else customer.name
        })
